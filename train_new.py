...
def main(opts):
    ...
    os.makedirs(join(opts.output_dir, 'results'), exist_ok=True)
    add_log_to_file(join(opts.output_dir, 'log', 'log.txt'))

    LOGGER.info(f"***** Running training with 1 GPUs *****")
                ...
                if global_step % opts.valid_steps == 0:
                    for split, loader in [('val', val_dataloader),
                        ...
                        with open(f'{opts.output_dir}/results/'
                                  f'{split}_results_{global_step}.csv', 'w') as f:
...
if __name__ == "__main__":
    ...
    if not args.force_output and \
       (exists(args.output_dir) and os.listdir(args.output_dir)):
