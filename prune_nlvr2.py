...
def main(opts):
    ...
    # ==========================================================================
    # Pruning Stuff
    valid_acc_pruning_threshold = (opts.full_model_valid_acc
                                   * opts.prune_valid_acc_threshold)
    sparsity_updated_this_epoch = False
    cur_sparsity = 0
    # ==========================================================================
            ...
            if (step + 1) % opts.gradient_accumulation_steps == 0:
                ...
                if global_step % opts.valid_steps == 0:
                    for split, loader in [('val', val_dataloader),
                        ...
                        if split == 'val':  # saving valid acc for pruning
                            valid_acc = log[f'valid/{split}_acc']
                    # ==========================================================================
                    # Pruning Stuff
                    if cur_sparsity < opts.sparsity_target \
                       and valid_acc > valid_acc_pruning_threshold:
                        cur_sparsity += opts.prune_rate
                        LOGGER.info(f"Pruning threshold reached: {valid_acc}")
                        LOGGER.info(f"Pruning new model sparisity: "
                                    f"{cur_sparsity}")
                        prune.ln_structured(model, amount=opts.rate,
                                            name="weight", n=2, dim=0)
                        model_saver.save(model, global_step)
                        sparsity_updated_this_epoch = True
                    # ==========================================================================
        if not sparsity_updated_this_epoch \
           and cur_sparsity < opts.sparsity_target:
            cur_sparsity += opts.prune_rate
            LOGGER.info(f"Pruning threshold reached: {valid_acc}")
            LOGGER.info(f"Pruning new model sparisity: "
                        f"{cur_sparsity}")
...
if __name__ == "__main__":
    ...
    # ==========================================================================
    # Pruning Stuff
    parser.add_argument("full_model_valid_acc", type=float,
                        help="valid acc of the full model")
    parser.add_argument("prune_valid_acc_threshold", type=float, default=0.975,
                        help="percent of full_model_valid_acc to start pruning")
    parser.add_argument("prune_rate", type=float, default=0.15,
                        help="ammount to prune at each iteration")
    parser.add_argument("sparsity_target", type=float,
